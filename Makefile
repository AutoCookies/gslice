...
run-server: build-go
	GPUSLICE_DB_PATH=./var/gpuslice.db GPUSLICE_SOCKET=/tmp/gpuslice.sock ./bin/gpusliced

demo: build
	@set -e; \
	DB=./var/demo.db; SOCK=/tmp/gpuslice-demo.sock; LOG=./var/demo-server.log; \
	mkdir -p ./var; rm -f $$SOCK $$DB; \
	GPUSLICE_DB_PATH=$$DB GPUSLICE_SOCKET=$$SOCK GPUSLICE_HTTP_ADDR=:18080 ./bin/gpusliced >$$LOG 2>&1 & \
	...
	LD_PRELOAD=$$(pwd)/native/interceptor/libgpuslice.so LD_LIBRARY_PATH=$$(pwd)/native/fakecuda GPUSLICE_SESSION=$$SESSION GPUSLICE_SOCKET=$$SOCK ./examples/target_app 4 1048576; \
